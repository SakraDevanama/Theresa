...
### 1. 卡牌描述自定义事件 (`DescriptionOverridePatches`)

允许其他 mod 通过事件自定义卡牌描述，无需继承或重写。适用于添加新的"关键词类效果"到描述中。

```csharp
using BaseLib.Patches.Content;

// 在游戏处理描述之前修改
DescriptionOverridePatches.CustomizeDescription += (card, target, ref description) =>
{
    if (card is MyCustomCard)
    {
        description += "\n自定义效果说明";
    }
};
...
// 在游戏处理描述之后修改
DescriptionOverridePatches.CustomizeDescriptionPost += (card, target, ref description) =>
{
    // 对最终描述做后处理
};
```

**注意**：此功能需要 `Publicize`（因为 Harmony 无法区分 `GetDescriptionForPile` 的重载）。

### 2. 自定义 Yummy Cookie (`CustomYummyCookie`)

自定义角色的 Yummy Cookie（遗物图标），覆盖原版 Yummy Cookie 的外观。

```csharp
public class MyCharacter : CustomCharacterModel
{
    public override YummyCookieData? CustomYummyCookie => new(
        BigIconPath: "res://MyMod/images/yummy_cookie_big.png",
        PackedIconPath: "res://MyMod/images/yummy_cookie_packed.png",
        PackedIconOutlinePath: "res://MyMod/images/yummy_cookie_outline.png"
    );
}
```

### 3. 自定义角色图标描边 (`CustomIconOutlineTexturePath`)
...
public class MyCharacter : CustomCharacterModel
{
    public override string? CustomIconOutlineTexturePath => 
        "res://MyMod/images/character_icon_outline.png";
}
```

### 4. ModelDb Hash 显示

调试模式下，屏幕左上角的 "MODDED" 标签会显示 ModelDb 的哈希值，用于多人游戏同步调试。

```
MODDED
ModelDbHash: a1b2c3d4
```

### 5. 移除 BaseLibSingleton

`BaseLibSingleton` 已被移除，其功能（`RefundVar` 能量返还、`Purge` 移除）改为 Harmony 补丁实现。如果你的代码引用了 `BaseLibSingleton`，需要移除相关引用。
